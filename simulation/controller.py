...
class RoutingController(object):
    ...
    def __init__(self):
        ...
        self.p4_src = self.get_p4_src()
        self.algorithm = sys.argv[2] if len(sys.argv) > 2 else ""
        self.is_ecmp_program = self.algorithm.startswith("ecmp")
        self.controllers = {}
        self.init()
    ...
    def get_p4_src(self):
        try:
            with open("topology.json", "r") as f:
                return os.path.basename(json.load(f).get("p4_src", ""))
        except Exception:
            return ""

    def init(self):
        print(f"controller algorithm={self.algorithm} p4_src={self.p4_src} ecmp_mode={self.is_ecmp_program}")
    ...
    def route(self):
        ...
        for sw_name, controller in self.controllers.items():
            ...
            if stype == 'c' :
                ...
                if self.is_ecmp_program:
                    controller.table_add("ipv4_lpm", "ecmp_group", ["10.0.0.0/24"], ["0", "%d" % half_k])
                else:
                    controller.table_add("ipv4_lpm", "ecmp_group", ["10.0.0.0/24"], ["%d" % half_k])

                for i in range(half_k) :
                    if self.is_ecmp_program:
                        controller.table_add("ecmp_group_to_nhop", "set_nhop", ["0", "%d" % i], ["%d" % (half_k + i + 1,)])
                    else:
                ...
                for i in range(half_k) :
                    if self.is_ecmp_program:
                        controller.table_add("ecmp_group_to_nhop", "set_nhop", ["0", "%d" % i], ["%d" % (i  + 1,)])
                    else:
